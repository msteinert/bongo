...
#include "bongo/range.h"
#include "bongo/unicode.h"

using namespace std::string_view_literals;

TEST_CASE("UTF-8 constants", "[unicode][utf8]") {
  CHECK(bongo::unicode::utf8::max_rune == bongo::unicode::max_rune);
  CHECK(bongo::unicode::utf8::rune_error == bongo::unicode::replacement_char);
}

TEST_CASE("UTF-8", "[unicode][utf8]") {
  auto test_cases = std::vector<std::tuple<bongo::rune, std::string_view>>{
    {0x0000, "\x00"sv},
    {0x0001, "\x01"},
    {0x007e, "\x7e"},
    {0x007f, "\x7f"},
  ...
  SECTION("Full rune") {
    for (auto [rune, s] : test_cases) {
      CAPTURE(rune, s);
      CHECK(bongo::unicode::utf8::full_rune(s.begin(), s.end()) == true);
      s = s.substr(0, s.size()-1);
      CHECK(bongo::unicode::utf8::full_rune(s.begin(), s.end()) == false);
    }
  ...
  SECTION("Encode") {
    for (auto [rune, s] : test_cases) {
      CAPTURE(s);
      std::string out;
      bongo::unicode::utf8::encode(rune, std::back_inserter(out));
      CHECK(out == s);
    }
  }
  SECTION("Decode") {
    for (auto [rune, s] : test_cases) {
      CAPTURE(rune, s);
      CHECK(
          bongo::unicode::utf8::decode(s.begin(), s.end())
          ==
          std::make_pair(rune, s.size()));

      // Try with trailing byte
      auto extra = std::string{s.begin(), s.end()} + '\0';
      CHECK(
          bongo::unicode::utf8::decode(extra.begin(), extra.end())
          ==
          std::make_pair(rune, s.size()));
      ...
      CHECK(
          bongo::unicode::utf8::decode(s.begin(), std::prev(s.end()))
          ==
          std::make_pair(bongo::unicode::utf8::rune_error, exp));
      ...
      CHECK(
          bongo::unicode::utf8::decode(bad.begin(), bad.end())
          ==
          std::make_pair(bongo::unicode::utf8::rune_error, 1ul));
    }
  }
  SECTION("Sequencing") {
    ...
    for (auto const& ts : test_strings) {
      for (auto [rune, view] : test_cases) {
        ...
        for (auto s : std::vector<std::string>{ts + str, str + ts, ts + str + ts}) {
          ...
          index.reserve(bongo::unicode::utf8::count(s.begin(), s.end()));
          size_t si = 0;
          for (auto [i, r] : bongo::range{s}) {
            CHECK(si == i);
            index.push_back({i, r});
            auto [r1, size] = bongo::unicode::utf8::decode(std::next(s.begin(), i), s.end());
            CHECK(r == r1);
            si += size;
          }
          si = s.size();
          for (auto it = std::rbegin(index); it < std::rend(index); ++it) {
            auto [r, size] = bongo::unicode::utf8::decode_last(s.begin(), std::next(s.begin(), si));
            si -= size;
...
TEST_CASE("Negative runes encode as U+FFFD", "[unicode][utf8]") {
  ...
  bongo::unicode::utf8::encode(bongo::unicode::utf8::rune_error, std::back_inserter(errorbuf));
  auto buf = std::vector<char>{};
  bongo::unicode::utf8::encode(-1, std::back_inserter(buf));
  CHECK(buf == errorbuf);
}
...
TEST_CASE("Count runes", "[unicode][utf8]") {
  ...
  for (auto [s, exp] : test_cases) {
    CAPTURE(s);
    CHECK(bongo::unicode::utf8::count(s.begin(), s.end()) == exp);
  }
}

TEST_CASE("Rune length", "[unicode][utf8]") {
  auto test_cases = std::vector<std::tuple<bongo::rune, int>>{
    {0, 1},
    {'e', 1},
    {0x00e9, 2},  // é
    {0x263a, 3},  // ☺
    {bongo::unicode::utf8::rune_error, 3},
    {bongo::unicode::utf8::max_rune, 4},
    {0xd800, -1},
    {0xdfff, -1},
    {bongo::unicode::utf8::max_rune + 1, -1},
    {-1, -1},
  };
  for (auto [rune, len] : test_cases) {
    CAPTURE(rune);
    CHECK(bongo::unicode::utf8::len(rune) == len);
  }
}
...
TEST_CASE("Valid rune", "[unicode][utf8]") {
  auto test_cases = std::vector<std::tuple<bongo::rune, bool>>{
    ...
    {0x00e9, true},  // é
    {0x263a, true},  // ☺
    {bongo::unicode::utf8::rune_error, true},
    {bongo::unicode::utf8::max_rune, true},
    {0xd7ff, true},
    {0xd800, false},
    {0xdfff, false},
    {0xe000, true},
    {bongo::unicode::utf8::max_rune + 1, false},
    {-1, false},
  };
  for (auto [r, exp] : test_cases) {
    CAPTURE(r);
    CHECK(bongo::unicode::utf8::valid(r) == exp);
  }
}
...
TEST_CASE("Valid range", "[unicode][utf8]") {
  ...
  for (auto [s, exp] : test_cases) {
    CAPTURE(s);
    CHECK(bongo::unicode::utf8::valid(s.begin(), s.end()) == exp);
  }
}
