...
TEST_CASE("UTF-16 constants", "[unicode/utf16]") {
  ...
  CHECK(replacement_char == unicode::replacement_char);
}

TEST_CASE("UTF-16 encode", "[unicode/utf16]") {
  auto test_cases = std::vector<std::tuple<std::vector<rune>, std::vector<uint16_t>>>{
    {{1, 2, 3, 4}, {1, 2, 3, 4}},
    {
  ...
  SECTION("Encode range") {
    for (auto [in, exp] : test_cases) {
      ...
      encode(in.begin(), in.end(), std::back_inserter(out));
      CHECK(out == exp);
    }
  }
  SECTION("Encode rune") {
    for (auto [in, exp] : test_cases) {
      ...
      for (auto r : in) {
        auto [r1, r2] = encode(r);
        if (r <0x10000 || r > unicode::max_rune) {
          CHECK(j < exp.size());
          CHECK(r1 == unicode::replacement_char);
          CHECK(r2 == unicode::replacement_char);
          ++j;
        } else {
          CHECK(j+1 < exp.size());
          CHECK(r1 == static_cast<rune>(exp[j]));
          CHECK(r2 == static_cast<rune>(exp[j+1]));
          CHECK(decode(r1, r2) == r);
          j += 2;
        }
      }
      CHECK(j == exp.size());
...
TEST_CASE("UTF-16 decode range", "[unicode/utf16]") {
  auto test_cases = std::vector<std::tuple<std::vector<uint16_t>, std::vector<rune>>> {
    {{1, 2, 3, 4}, {1, 2, 3, 4}},
    {
      {0xffff, 0xd800, 0xdc00, 0xd800, 0xdc01, 0xd808, 0xdf45, 0xdbff, 0xdfff},
      {0xffff, 0x10000, 0x10001, 0x12345, 0x10ffff}
...
TEST_CASE("UTF-16 decode rune", "[unicode/utf16]") {
  auto test_cases = std::vector<std::tuple<rune, rune, rune>>{
