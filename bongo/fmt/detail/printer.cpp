...
auto printer::width() const -> std::pair<int, bool> {
  return {fmt_.wid, fmt_.flags.wid_present};
}

auto printer::precision() const -> std::pair<int, bool> {
  return {fmt_.prec, fmt_.flags.prec_present};
}

auto printer::flag(int b) const -> bool {
  switch (b) {
  case '-':
...
auto printer::write(std::span<uint8_t const> b) -> std::pair<int, std::error_code> {
  buf_.write(b);
  return {b.size(), nil};
}
