...
constexpr std::pair<uint32_t, uint32_t> hash(std::string_view sep) {
  uint32_t h = 0;
  for (auto c : sep) {
...
constexpr std::string_view::size_type index_rabin_karp(std::string_view s, std::string_view substr) {
