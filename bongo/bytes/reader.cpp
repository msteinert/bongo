...
std::pair<int, std::error_code> reader::read(std::span<uint8_t> p) {
  ...
  prev_rune_ = -1;
  auto s = std::span<uint8_t const>{std::next(s_.begin(), i_), s_.end()};
...
std::pair<int, std::error_code> reader::read_at(std::span<uint8_t> p, int64_t off) {
  ...
  auto s = std::span<uint8_t const>{std::next(s_.begin(), off), s_.end()};
  auto n = std::min(s.size(), p.size());
  std::copy_n(s.begin(), n, p.begin());
  if (n < p.size()) {
    return {n, io::eof};
...
void reader::reset(std::span<uint8_t const> s) {
  s_ = s;
  i_ = 0;
  prev_rune_ = -1;
}
